...
DICT_EMPTY_BOARDS: {int: {int: [int, [int]]}} = {
    EMPTY_BOARD: {EMPTY_BOARD: [0, [3, 3, 3, 3, 3]]}}  # usage for presentation
...
def to_array(bitboard: int) -> np.ndarray:
    ...
    return np.rot90(np.fromiter(bitboard_string, dtype=int).reshape(BOARD_SHAPE_BINARY),
                    3)  # read to numpy array and reshape to fit board
...
def pretty_print_board(board_player_one: int, board_player_two: int) -> str:
    ...
    for index in range(1, HEIGHT_BOARD + 1):
        body += SIDE_PRETTY_PRINT_BOARD + ''.join(
            map('{} '.format, board_modified[index])) + SIDE_PRETTY_PRINT_BOARD + "\n"
...
def string_to_board(pp_board: str) -> tuple[int, int]:
    ...
    entry_array = list(map(get_entries, string_input))  # containing numbers
    entry_array = np.vstack((EMPTY_ROW_CHAR, entry_array))  # add buffer
    entry_array = np.rot90(entry_array, 1)

    # array to ints
    replace_board: Callable = np.vectorize(lambda dictionary, key: dictionary.get(key))
    entry_array_numbers_player_one: np.ndarray = replace_board(PRINT_BACK_SUBSTITUTION_TABLE_PLAYER_ONE,
                                                               entry_array).flatten()
    output_player_one: int = int(np.array2string(entry_array_numbers_player_one, separator='')[1:-1], 2)
    entry_array_numbers_player_two: np.ndarray = replace_board(PRINT_BACK_SUBSTITUTION_TABLE_PLAYER_TWO,
                                                               entry_array).flatten()
...
def apply_player_action(board_player_one: int, board_player_two: int, player: BoardPiece, action: PlayerAction) -> \
        tuple[int, int]:
    """
    ...
    """
    move_board = ((board_player_one | board_player_two) + (
                1 << action * (HEIGHT_BOARD + 1)))  # all positions plus current action
...
def get_possible_moves(board_player_one: int, board_player_two: int, player: BoardPiece, next_move: int = 3) -> (
    ...
    """
    game_state: GameState = check_end_state(board_player_one=board_player_one, board_player_two=board_player_two,
                                            player=3 - player)  # win needs to be checked on the other players board
